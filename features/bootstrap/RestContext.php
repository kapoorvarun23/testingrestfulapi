...
class RestContext extends BehatContext
{
	...
	*/
	public function thatIWantToFindA($objectType)
	{
	 ...
	 /**
     * @Given /^that I want to authenticate a "([^"]*)"$/
     */
	public function thatIWantToAuthenticateA($objectType)
	{
		$this->_restObjectType = ucwords(strtolower($objectType));
		$this->_restObjectMethod = 'post';
	}
	...
	*/
	public function iRequest($pageUrl)
	{
		...
		switch (strtoupper($this->_restObjectMethod)) {
			...
			$response = $this->_client
			->post($this->_requestUrl,['json'=>$postFields]);
			break;
			case 'DELETE':
			$response = $this->_client
	...
	*/
	...
	{
		$data = json_decode($this->_response->getBody(true));
		$objType = $this->_restObjectType;
		if (!empty($data)) {
			if (!isset($data->$objType->$propertyName)) {
